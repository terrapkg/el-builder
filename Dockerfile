...
RUN dnf install -y --nogpgcheck --repo=terra terra-gpg-keys && \
    dnf install -y --repo=extras epel-release && \
    dnf up -y && \
    dnf install -y \
terra-mock-configs terra-appstream-helper terra-scripts redhat-rpm-config epel-rpm-macros almalinux-release adoptium-temurin-java-repository \
